  ...
  public void dirct(final Director dir, final String[] sql) {
    ...
    if (dir.getExtract().equals("select")) {
      ...
      if (extractor.getCol(sql) == null) {
        ...
        handler.createTable(extractor.getDataBaseName(sql),
            extractor.getTableName(sql), extractor.getCol(sql));
      }
      ...
      final ArrayList<Integer> index = x
          .parseCondition(Methods.getXmlFile(UsedDataBase.getUsedDataBase(),
              extractor.getTableName(sql)), extractor.getCondition(sql));
      ...
      handler.delete(Methods.getXmlFile(UsedDataBase.getUsedDataBase(),
          extractor.getTableName(sql)), indexes);
      ...
      if (extractor.getTableName(sql)== null) {
        ...
        handler.dropTable(Methods.getXmlFile(UsedDataBase.getUsedDataBase(),
            extractor.getTableName(sql)));
      }

    } else if (dir.getExtract().equals("update")) {
      handler.update(
          Methods.getXmlFile(extractor.getDataBaseName(sql),
              extractor.getTableName(sql)),
          extractor.getCol(sql), extractor.getData(sql),
          extractor.getCondition(sql));
    } else if (dir.getExtract().equals("insert")) {
      handler.insertIntoTable(
          Methods.getXmlFile(UsedDataBase.getUsedDataBase(),
              extractor.getTableName(sql)),
          extractor.getCol(sql), extractor.getData(sql));
