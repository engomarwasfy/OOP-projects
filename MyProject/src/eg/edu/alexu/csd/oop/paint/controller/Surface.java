...
public class Surface extends JPanel {
	...
	/**
	 * All moves of paint program.
	 * object of class undo redo.
	 */
		...
		try{
			...
			if(startDrag!=null&&endDrag!=null){
				if( Gui.activeButton == "rectangle") {
					...
					new SquareImp(startDrag.x, startDrag.y, endDrag.x, startDrag.y + (endDrag.x - startDrag.x),null,getColor()).draw(g, false);
					...
					new CircleImp(startDrag.x, startDrag.y, endDrag.x, startDrag.y + (endDrag.x - startDrag.x),null,getColor()).draw(g, false);
				} else if( Gui.activeButton == "triangle") {
					
				 try {
					 DynamicLoader x = new DynamicLoader();
					x.getShape(url,startDrag.x, startDrag.y, endDrag.x, endDrag.y,null,getColor()).draw(g, false);
			...
			try{
				...
				if (Gui.activeButton == "select") {
					...
					if(Gui.activeButton == "delete") {
						...
						isSelected = false;
					} else if ((selectedShape != null) && !selectedShape.isLine()&&(selectedShape.getShape().contains(e.getPoint())) && (Gui.activeButton == "fillcolor")) {
						...
						m.takeMove(s);
						isSelected = false;
					} else if( (selectedShape != null) && !selectedShape.isLine()&&(selectedShape.getShape().contains(e.getPoint())) && (Gui.activeButton == "drawcolor")) {
		...
		public void mouseReleased(final MouseEvent e) {
			...
			if (Gui.activeButtonEdit != "edit") {
				if( Gui.activeButton == "rectangle") {
					...
					shapes.add(new SquareImp(startDrag.x, startDrag.y, endDrag.x, startDrag.y + (endDrag.x - startDrag.x),null,getColor()));
				} else if( Gui.activeButton == "elipse") {
					shapes.add(new ElipseImp(startDrag.x , startDrag.y , endDrag.x , endDrag.y,null,getColor()));
				} else if( Gui.activeButton == "circle") {
					shapes.add(new CircleImp(startDrag.x, startDrag.y, endDrag.x, startDrag.y + (endDrag.x - startDrag.x),null,getColor()));
				} else if( Gui.activeButton == "triangle") {
					 DynamicLoader x = new DynamicLoader();
					try {
						shapes.add(x.getShape(url,startDrag.x, startDrag.y, endDrag.x, endDrag.y,null,getColor()));
		...
		public void mouseDragged(final MouseEvent e) {
			...
			if (Gui.activeButtonEdit == "edit") {
				...
				if(selectedShape!= null &&!selectedShape.isLine()) {
					if (selectedShape != null && selectedShape.getShape().contains(e.getPoint()) && Gui.activeButton == "move") {
						isSelected = true;
