...
public class ExpressionEvaluator {
	...
	if (expression.contains(" ")) {
		...
		if (checkoperator(express[count]) && checkoperator(express[count + 2]) && count < express.length - 2) {
		    throw new RuntimeException("invalid");
		}
		if (operation.size() >= 2) {
	    ...
	    for (count = 0; count < express.length; count++) {
		if (checkoperator(express[count]) && checkoperator(express[count + 1]) && count < express.length - 1) {
		    throw new RuntimeException("invalid");
		}
		if (operation.size() >= 2) {
		    char test = (char) operation.pop();
