...
public abstract class Validator {
	...
	for (int i = 0; i < arr.length; i++) {
	    if (!isValidName(arr[i]) && !isIntNumber(arr[i]) && !isFloatNumber(arr[i]) && !isDate(arr[i])) {
		return false;
	    }
	}
	return true;
	...
	return (isValidName(leftOperand.toString()))
		&& (isValidName(rightOperand) || isIntNumber(rightOperand)||isDate(rightOperand)||isFloatNumber(rightOperand));
	...
	if((dataType.equalsIgnoreCase("int")) || (dataType.equalsIgnoreCase("varchar")) || (dataType.equalsIgnoreCase("date")|| (dataType.equalsIgnoreCase("float")) )) {
	    return true;
	}
	return false;
